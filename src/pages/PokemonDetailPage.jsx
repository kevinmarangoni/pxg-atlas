...
function membershipLabel(membership, t) {
  return membership.section || membership.category_label || t('Classificação publicada')
}
  ...
  return (
    <article className="membership-card">
      <div className="membership-heading">
        <div><span>{membership.clan}</span><strong>{membershipLabel(membership, t)}</strong></div>
        {tier && <b className="large-tier" title={t('Tier {tier}', { tier })}>{tier}</b>}
      </div>
          ...
          return (
            <div className={`mode-block ${mode} ${available ? '' : 'unavailable'}`} key={mode}>
              <div className="mode-block-heading">
                ...
                <small>{available ? t('Disponível') : t('Indisponível')}</small>
              </div>
              {available ? (
                <div className="role-stack">
                  ...
                  {!roles.length && !unknownRoles.length && <em>{t('Função não catalogada')}</em>}
                </div>
              ) : <em>{t('Não consta para este modo')}</em>}
      ...
      {membership.pvp_exclusive && <div className="exclusive-note"><ShieldCheck size={14} />{t('Exclusivo deste clan no PvP')}</div>}
    </article>
  )
}
...
function ClassificationCard({ memberships, clans, tiers }) {
  ...
  return (
    <article className="decision-card classification-card">
      <header><span className="decision-icon"><Crown size={19} /></span><div><small>{t('Classificação')}</small><strong>{t('Clan e tier')}</strong></div></header>
      {memberships.length > 0 ? (
        <>
          <div className="classification-tiers">{tiers.map((tier) => <b key={tier}>{tierLabel(tier)}</b>)}</div>
          <p>{clans.join(' · ')}</p>
          <span className="decision-footnote">{memberships.length === 1 ? t('{count} entrada publicada', { count: memberships.length }) : t('{count} entradas publicadas', { count: memberships.length })}</span>
        </>
      ) : (
        <div className="decision-empty"><strong>{t('Sem classificação publicada')}</strong><p>{t('Este Pokémon não aparece nas tabelas de clans da Wiki.')}</p></div>
      )}
    </article>
  )
}
...
function CombatDecisionCard({ mode, summary, hasMemberships, roleCatalog }) {
  ...
  const modeLabel = mode === 'pve' ? t('PvE') : t('PvP')
  return (
    <article className={`decision-card combat-decision ${mode}`}>
      <header>
        ...
        <div><small>{t('Função {mode}', { mode: modeLabel })}</small><strong>{available ? t('Disponível') : hasMemberships ? t('Não disponível') : t('Sem dados de clan')}</strong></div>
      ...
      {available ? (
        <>
          <div className="decision-role-list">
            ...
            {!summary.roles.length && <span className="unknown-role">{summary.rawLabels.join(' · ') || t('Função não catalogada')}</span>}
          </div>
          {summary.heldItems.length > 0 && (
            <div className="decision-helds"><small>{t('Held items publicados')}</small><div>{summary.heldItems.map((item, index) => <span key={`${item.name}-${index}`}>{item.icon_url && <img src={item.icon_url} alt="" />}<b>{item.name}</b></span>)}</div></div>
          )}
          <span className="decision-footnote">{t('Em {clans}', { clans: summary.clans.join(' · ') })}</span>
        </>
      ) : (
        <div className="decision-empty">
          <p>{hasMemberships ? t('A tabela não marca este Pokémon como disponível no {mode}.', { mode: modeLabel }) : t('É preciso uma classificação de clan para avaliar este modo.')}</p>
        </div>
      )}
    </article>
  )
...
function PokelogDecisionCard({ pokemon }) {
  ...
  return (
    <article className="decision-card pokelog-decision">
      <header><span className="decision-icon"><ClipboardList size={19} /></span><div><small>{t('Progressão')}</small><strong>Pokélog {pokelog.category}</strong></div></header>
      <div className="pokelog-decision-meta"><span>XP {pokelog.experience_category}</span><span>{t('{count} estágios', { count: stages.length })}</span></div>
      <p><small>{t('Meta final')}</small><strong>{formatNumber(finalStage.quantity_raw || finalStage.required_quantity, locale)}</strong></p>
      <span className="decision-footnote">{t('{count} pontos no último estágio', { count: formatNumber(finalStage.points_raw || finalStage.points, locale) })}</span>
    </article>
  )
}
...
function UtilityDecisionCard({ abilities }) {
  ...
  return (
    <article className="decision-card utility-decision">
      <header><span className="decision-icon"><Package size={19} /></span><div><small>{t('Fora do combate')}</small><strong>{t('Utilidade')}</strong></div></header>
      <div className="utility-chips">{abilities.map((ability) => <span key={ability}>{ability}</span>)}</div>
      <span className="decision-footnote">{t('Habilidades publicadas na ficha')}</span>
    </article>
  )
}
...
function CaptureDecisionCard({ pokemon, captureBallCatalog }) {
  ...
  return (
    <article className="decision-card capture-decision">
      <header><span className="decision-icon"><Gauge size={19} /></span><div><small>{officialDifficulty ? t('Dificuldade oficial') : t('Médias de captura')}</small><strong>{officialDifficulty?.label || t('Sem classificação oficial')}</strong></div></header>
      {reference ? (
        <div className="capture-decision-average">
          {reference.image_url ? <img src={reference.image_url} alt="" /> : <CircleDot size={28} />}
          <span><small>{reference.name}</small><strong>{formatNumber(reference.average, locale)}</strong><em>{t('balls em média')}</em></span>
        </div>
      ) : <div className="decision-empty"><p>{t('Não há média positiva publicada para este registro.')}</p></div>}
      <span className="decision-footnote">{officialDifficulty ? t('Classificação do Gerenciamento de Captura · ') : ''}{t('A média não representa garantia.')}</span>
    </article>
  )
}
...
function DecisionOverview({ pokemon, roleCatalog, captureBallCatalog }) {
  ...
  return (
    <Section id="overview" title={t('Resumo para decisão')} icon={<Target size={18} />} description={t('Onde este Pokémon se encaixa, sem notas ou rankings inventados.')}>
      ...
      <div className="capture-ball-grid">
        {balls.map((ball) => (
          <article className="capture-ball-card" key={ball.id}>
            ...
            <div><small>{ball.name}</small><strong>{formatNumber(ball.average, locale)}</strong><span>{Number(ball.average) === 1 ? t('ball em média') : t('balls em média')}</span></div>
          </article>
        ))}
      </div>
    </div>
...
function CaptureSection({ pokemon, catalog, metadata }) {
  ...
  return (
    <Section id="capture" title={t('Médias de captura')} icon={<Gauge size={18} />} description={t('Quantidade média de balls publicada pelo projeto comunitário media-pxg.')}>
      <div className="capture-summary">
        <div className="capture-classification">
          <small>{t('Dificuldade oficial')}</small>
          <strong className={`capture-difficulty ${difficultyId}`}>{capture.difficulty?.label || t('Não identificada')}</strong>
          <span>{capture.difficulty ? t('Classificação exibida pelo Gerenciamento de Captura do PXG.') : t('Este registro não apareceu na transcrição das listas oficiais fornecidas.')}</span>
          {metadata?.official_difficulty?.documentation_url && <SourceLink href={metadata.official_difficulty.documentation_url}>{t('Entender o sistema')}</SourceLink>}
        </div>
        <div className="capture-recommendation">
          <small>{t('Balls recomendadas pela fonte')}</small>
          {recommended.length > 0 ? <div>{recommended.map((ball) => <b key={ball}>{publishedBallLabel(ball, catalog)}</b>)}</div> : <strong>{t('Nenhuma recomendação')}</strong>}
        </div>
      </div>

      {balls.length > 0 ? (
        <div className="capture-groups">
          <CaptureBallGroup title={t('Balls padrão')} balls={standard} />
          <CaptureBallGroup title={t('Balls especiais')} balls={special} />
        </div>
      ) : (
        <EmptyDetailState icon={<CircleDot size={20} />} title={t('Sem médias positivas')}>{t('A fonte preserva este Pokémon, mas não publica uma quantidade média utilizável.')}</EmptyDetailState>
      )}

      <div className="capture-source-note">
        <Gauge size={17} />
        <p><strong>{t('Use como referência, não como promessa.')}</strong> {metadata?.warning || t('Os valores são estimativas históricas da comunidade e podem estar desatualizados.')}</p>
        <SourceLink href={metadata?.repository_url}>{t('Ver fonte')}</SourceLink>
      </div>
    </Section>
  )
}
...
function PokelogSection({ pokemon }) {
  ...
  const rawOrValue = (raw, value) => raw || formatNumber(value, locale)

  return (
    <Section id="pokelog" title={t('Progresso no Pokélog')} icon={<ClipboardList size={18} />} description={t('Marque seus objetivos; o progresso fica salvo somente neste navegador.')}>
      <div className="pokelog-summary">
        <div><span>{t('Categoria')}</span><strong>{pokelog.category}</strong></div>
        <div><span>{t('Experiência')}</span><strong>{pokelog.experience_category}</strong></div>
        <div className="pokelog-progress-copy"><span>{t('Seu progresso')}</span><strong>{t('{done}/{total} estágios · {percent}%', { done, total: stages.length, percent })}</strong></div>
      </div>
      <div className="pokelog-progress" aria-label={t('{percent}% concluído', { percent })}><i style={{ width: `${percent}%` }} /></div>
      {pokemon.scrape_status === 'not_published' && <p className="pokelog-note">{t('Esta forma existe no compilado do Pokélog, mas ainda não possui uma ficha individual publicada na Wiki.')}</p>}
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <article className={`pokelog-stage ${isDone ? 'completed' : ''}`} key={`${stage.stage}-${index}`}>
              <header>
                <div><small>{t('Estágio')}</small><strong>{stage.stage}</strong></div>
                <label className="pokelog-check">
                  <input type="checkbox" checked={isDone} onChange={() => toggle(stage, index)} aria-label={t('Marcar estágio {stage} como concluído', { stage: stage.stage })} />
                  <span><Check size={15} /></span>
                </label>
              </header>
              <div className="stage-target"><small>{t('Quantidade')}</small><strong>{rawOrValue(stage.quantity_raw, stage.required_quantity)}</strong></div>
              <footer><span><small>{t('Pontos')}</small><b>{rawOrValue(stage.points_raw, stage.points)}</b></span><span><small>{t('Experiência')}</small><b>{rawOrValue(stage.experience_raw, stage.experience)}</b></span></footer>
            </article>
          )
        })}
      </div>
      {done > 0 && <button type="button" className="text-action" onClick={() => setPokelogStages(id, [])}>{t('Limpar progresso')}</button>}
    </Section>
  )
}
...
function BerryRecommendationsSection({ pokemon, berries = [] }) {
  ...
  return (
    <Section id="berries" title={t('Berries úteis')} icon={<Sprout size={18} />} description={t('Sugestões derivadas das efetividades e dos efeitos de controle publicados nesta ficha.')}>
      <div className="pokemon-berry-grid">{recommendations.map(({ berry, elemental, statuses }) => <Link to="/berries" key={berry.id}>{berry.image_url && <img src={berry.image_url} alt="" />}<div><strong>{berry.name}</strong><p>{berry.effect}</p><small>{elemental.length ? t('Proteção: {elements}', { elements: elemental.join(', ') }) : t('Status: {statuses}', { statuses: statuses.join(', ') })}</small></div></Link>)}</div>
    </Section>
  )
}
...
function PokemonNpcObtainedSection({ entries, metadata }) {
  ...
  return (
    <Section id="npc-obtained" title={t('Obtido via NPC')} icon={<UserRound size={18} />} description={t('Requisitos, custos e locais publicados pela Wiki oficial para obter esta forma.')}>
        ...
        {entries.map((entry) => {
          ...
          return (
            <article className="pokemon-npc-obtained-card" key={entry.id}>
              <header><div><small>{entry.category || t('Obtenção publicada')}</small><strong>{entry.title}</strong></div><b>{entry.reward || entry.pokemon}</b></header>
              <div className="pokemon-npc-obtained-facts">
                <span><UserRound size={13} /><small>NPC</small><strong>{entry.npc || t('Não informado')}</strong></span>
                <span><MapPin size={13} /><small>{t('Local')}</small><strong>{entry.location || t('Não informado')}</strong></span>
              </div>
              <div className="pokemon-npc-obtained-flow">
                <div><small>{t('Entregar / cumprir')}</small><p>{delivered.length ? delivered.join(' · ') : t('Requisito não informado')}</p></div>
                <div><small>{t('Custo')}</small><p>{entry.cost?.length ? entry.cost.join(' · ') : t('Sem custo informado')}</p></div>
              </div>
              {entry.notes?.length > 0 && <details><summary>{t('Restrições e observações')}</summary><ul>{entry.notes.map((note, index) => <li key={`${entry.id}-note-${index}`}>{note}</li>)}</ul></details>}
            </article>
          )
        })}
      </div>
      {metadata?.source_url && <p className="pokemon-npc-source"><SourceLink href={metadata.source_url}>{t('Ver página oficial de obtenção')}</SourceLink><span>{t('Revisão {id}', { id: metadata.revision_id || '—' })}</span></p>}
    </Section>
  )
}
...
function TaskOccurrencesSection({ occurrences, tasksById }) {
  ...
  return (
    <Section id="tasks" title={t('Tasks relacionadas')} icon={<ClipboardList size={18} />} description={t('Veja onde este Pokémon aparece como objetivo e abra a task completa para consultar todos os passos e recompensas.')}>
      <div className="pokemon-task-region-filter">
        <span>{t('Filtrar por região')}</span>
        <div role="group" aria-label={t('Filtrar tasks relacionadas por região')}>
          <button type="button" className={effectiveRegion === 'all' ? 'active' : ''} aria-pressed={effectiveRegion === 'all'} onClick={() => setRegionFilter('all')}>{t('Todas')}<b>{grouped.length}</b></button>
        ...
        <small>{t('{visible} de {total} {label}', { visible: visibleGroups.length, total: grouped.length, label: grouped.length === 1 ? 'task' : 'tasks' })}</small>
      </div>
        ...
        {visibleGroups.map(({ task, occurrences: taskOccurrences }) => {
          ...
          return (
            <article className={`pokemon-task-card region-${task?.region || fallback.region}`} key={fallback.task_id}>
              ...
              <div className="pokemon-task-occurrences">
                {taskOccurrences.map((occurrence, index) => (
                  <div key={`${occurrence.step_number}-${occurrence.raw_target}-${index}`}>
                    <span><Route size={13} />{t('Passo {step}', { step: occurrence.step_number })}</span>
              ...
              {task?.rewards?.length > 0 && <div className="pokemon-task-rewards"><span><Gift size={13} />{t('Recompensas')}</span><p>{task.rewards.slice(0, 3).map((reward, index) => <b key={`${reward.name}-${index}`}>{reward.image_url && <img src={reward.image_url} alt="" loading="lazy" />}{reward.raw || reward.name}</b>)}{task.rewards.length > 3 && <em>+{task.rewards.length - 3}</em>}</p></div>}
              <Link to={`/tasks?task=${encodeURIComponent(fallback.task_id)}`}>{t('Abrir task completa')}<ChevronRight size={14} /></Link>
            </article>
          )
        })}
      </div>
...
function MovesSection({ moves }) {
  ...
  const labels = { default: onlyDefault ? t('Moveset compartilhado') : t('Padrão'), pve: 'PvE', pvp: 'PvP' }
  ...
  return (
    <Section id="moves" title={t('Kit de movimentos')} icon={<Zap size={18} />} description={t('Compare slots, elemento, level necessário e cooldown; a Wiki não publica dano ou DPS.')}>
      ...
      <div className="move-kit-summary">
        <div><small>{t('Moveset selecionado')}</small><strong>{t('{count} movimentos', { count: activeMoves.length })}</strong></div>
        {tagSummary.length > 0 && <div className="move-feature-list"><small>{t('Características mais presentes')}</small><p>{tagSummary.map(([tag, count]) => <span key={tag}>{tag}<b>{count}</b></span>)}</p></div>}
      </div>
      <div id="moves-panel" role="tabpanel" aria-labelledby={`moves-tab-${active}`}>
        <div className="moves-table-wrap">
          <table className="moves-table">
            <thead><tr><th>Slot</th><th>{t('Movimento')}</th><th>{t('Elemento')}</th><th>Level</th><th>Cooldown</th><th>{t('Características')}</th></tr></thead>
            <tbody>
...
function EffectivenessSection({ name, effectiveness }) {
  ...
  return (
    <Section id="effectiveness" title={t('Efetividade elemental')} icon={<ShieldCheck size={18} />} description={t('Como a Wiki classifica ataques e interações elementais para {name}.', { name })}>
      <div className="effectiveness-legend"><span><i />{t('Mais efetivo')}</span><span><i />{t('Neutro')}</span><span><i />{t('Mais resistente')}</span></div>
        ...
        {rows.map(({ key, values }) => {
          ...
          return (
            <article className={`effectiveness-group ${meta.tone}`} key={key}>
              ...
              {!EFFECTIVENESS_LABELS[key] && <p>{t('Regra especial preservada como publicada pela Wiki.')}</p>}
            </article>
          )
        })}
      </div>
...
function MapLocationsSection({ name, locations, cdnHome, tilePositionSet, localTilePositionSet, localTileHome, mapSources }) {
  ...
  const regionOptions = useMemo(() => {
    ...
    groupedLocations.forEach((location) => {
      const region = String(location.region || t('Região não informada')).trim()
  ...
  return (
    <Section id="locations" title={t('Onde encontrar')} icon={<MapPin size={18} />} description={t('Respawns de {name}; coordenadas próximas na mesma região e andar são consolidadas.', { name })}>
      <div className="pokemon-location-summary">
        <div><strong>{filteredLocations.length}</strong><span>{filteredLocations.length === 1 ? t('respawn') : t('respawns')}<small> · {filteredPointCount === 1 ? t('{count} ponto mapeado', { count: filteredPointCount }) : t('{count} pontos agrupados', { count: filteredPointCount })}</small></span></div>
        <Link to={mapUrl}>{t('Explorar no mapa')}<ChevronRight size={14} /></Link>
      </div>
      <div className="pokemon-location-filters" role="group" aria-label={t('Filtrar respawns por região')}>
        <button type="button" className={selectedRegion === 'all' ? 'active' : ''} aria-pressed={selectedRegion === 'all'} onClick={() => setSelectedRegion('all')}>{t('Todos')}<b>{groupedLocations.length}</b></button>
      ...
      <div className="pokemon-location-grid">
        {filteredLocations.slice(0, 8).map((location, index) => (
          <button type="button" className={selectedIndex === index ? 'selected' : ''} aria-pressed={selectedIndex === index} onClick={() => setSelectedIndex(index)} key={`${location.x}-${location.y}-${location.z}-${index}`}>
            ...
            <div><small>{location.region} · {t('Andar {floor}', { floor: location.floor ?? location.z })}{location.point_count > 1 ? ` · ${t('{count} pontos', { count: location.point_count })}` : ''}</small><strong>{location.x.toLocaleString(locale)}, {location.y.toLocaleString(locale)}, {location.z}</strong>{location.comment && <p>{location.comment}</p>}</div>
            <ChevronRight size={14} />
          </button>
        ))}
      </div>
      {filteredLocations.length > 8 && <p className="pokemon-location-more">{t('Mais {count} respawns disponíveis no mapa interativo.', { count: filteredLocations.length - 8 })}</p>}
    </Section>
  )
}
...
function CounterCard({ recommendation }) {
  ...
  return (
    <Link className="counter-card" to={pokemonPath(pokemon)} aria-label={t('Abrir ficha de {name}', { name })}>
      ...
      <div className="counter-card-copy">
        <div className="counter-card-heading">
          <span>{level ? t('Level {level}', { level }) : t('Level não informado')}</span>
          <strong>{name}</strong>
        </div>
        <div className="counter-attack-row">
          <small>{t(effectivenessLabel)}</small>
          <div>{attackElements.map((element) => <ElementBadge key={element} element={element} compact />)}</div>
        </div>
        <span className={`counter-defense ${defense.id}`}><i />{t(defense.label)}</span>
      ...
      {visible.length > 0 ? (
        <>
          ...
          <div className="counter-group-actions">
            {visibleCount > COUNTER_PAGE_SIZE && <button type="button" onClick={onShowLess}>{t('Mostrar menos')}</button>}
            {recommendations.length > visibleCount && <button type="button" onClick={onShowMore}>{t('Mostrar mais {count}', { count: Math.min(COUNTER_PAGE_SIZE, recommendations.length - visibleCount) })}</button>}
          </div>
        </>
      ) : <p className="counter-group-empty">{t('Nenhum Pokémon desta categoria corresponde aos filtros.')}</p>}
    </div>
  )
}
...
function CounterRecommendationsSection({ target, pokemon }) {
  ...
  return (
    <Section id="counters" title={t('Pokémon indicados contra este alvo')} icon={<Target size={18} />} description={t('Sugestões calculadas pelos elementos dos movimentos e pela sensibilidade publicada na ficha; a defesa ajuda a ordenar as opções, mas não muda a categoria.')}>
      {!weaknesses.length ? (
        <EmptyDetailState icon={<ShieldCheck size={20} />} title={t('Sem sensibilidades ofensivas publicadas')}>{t('A ficha não informa elementos efetivos, muito efetivos ou super efetivos para montar esta lista.')}</EmptyDetailState>
      ) : (
        <>
          <div className="counter-method-note">
            <span>{t('Como é calculado')}</span>
            <p><strong>{t('Muito indicados')}</strong> {t('usam movimentos muito ou super efetivos.')} <strong>{t('Indicados')}</strong> {t('usam movimentos efetivos. Dentro de cada grupo, quem lida melhor com os ataques do alvo aparece primeiro.')}</p>
          </div>
          <div className="counter-toolbar">
            <div className="counter-mode-switch" role="group" aria-label={t('Modo de combate')}>
            ...
            <label className="counter-search"><span>{t('Buscar Pokémon')}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ex.: Charizard" /></label>
            <label><span>{t('Elemento de ataque')}</span><select value={attackElement} onChange={(event) => setAttackElement(event.target.value)}><option value="">{t('Todos')}</option>{weaknesses.map(({ element }) => <option value={element} key={element}>{element}</option>)}</select></label>
            <label className="counter-level"><span>{t('Meu level')}</span><input type="number" min="1" max="999" inputMode="numeric" value={maxLevel} onChange={(event) => setMaxLevel(event.target.value)} placeholder={t('Todos')} /></label>
            <button type="button" className={`counter-safe-filter ${safeOnly ? 'active' : ''}`} aria-pressed={safeOnly} onClick={() => setSafeOnly((value) => !value)}><ShieldCheck size={15} />{t('Sem desvantagem')}</button>
            {activeFilterCount > 0 && <button type="button" className="counter-reset" onClick={resetFilters}>{t('Limpar {count}', { count: activeFilterCount })}</button>}
          </div>
          <div className="counter-result-summary"><strong>{filtered.length}</strong> {filtered.length === 1 ? t('opção encontrada') : t('opções encontradas')} {t('para {mode}', { mode: mode.toUpperCase() })}</div>
          <div className="counter-groups">
            <CounterGroup id="strong" title={t('Muito indicados')} description={t('Movimentos muito ou super efetivos contra este Pokémon.')} recommendations={groups.strong} visibleCount={visibleCounts.strong} onShowMore={() => showMore('strong')} onShowLess={() => showLess('strong')} />
            <CounterGroup id="indicated" title={t('Indicados')} description={t('Movimentos efetivos contra este Pokémon.')} recommendations={groups.indicated} visibleCount={visibleCounts.indicated} onShowMore={() => showMore('indicated')} onShowLess={() => showLess('indicated')} />
          </div>
        </>
      )}
    </Section>
...
function DetailPager({ previous, next }) {
  const { t } = useLanguage()
  return (
    <nav className="detail-pager" aria-label={t('Navegar entre Pokémon')}>
      {previous ? (
        <Link className="previous" to={pokemonPath(previous)} aria-label={t('Pokémon anterior: {name}', { name: displayName(previous) })}>
          <ChevronLeft size={18} /><span><small>{t('Anterior')}</small><strong>{displayName(previous)}</strong></span>
        </Link>
      ) : <span className="pager-placeholder" />}
      {next ? (
        <Link className="next" to={pokemonPath(next)} aria-label={t('Próximo Pokémon: {name}', { name: displayName(next) })}>
          <span><small>{t('Próximo')}</small><strong>{displayName(next)}</strong></span><ChevronRight size={18} />
        </Link>
      ) : <span className="pager-placeholder" />}
    </nav>
  )
...
export default function PokemonDetailPage() {
  ...
  if (!entry) {
    return <div className="not-found"><BackLink /><h1>{t('Pokémon não encontrado')}</h1><p>{t('Este registro não existe na base atual.')}</p></div>
  }
  ...
  const sectionLinks = [
    ...
    hasEffectiveness && { id: 'counters', label: t('Indicações') },
    catalogData?.berries?.length > 0 && { id: 'berries', label: t('Berries') },
    hasMoves && { id: 'moves', label: t('Movimentos') },
    hasPokelog && { id: 'pokelog', label: 'Pokélog' },
    hasTasks && { id: 'tasks', label: t('Tasks ({count})', { count: new Set(taskOccurrences.map((occurrence) => occurrence.task_id)).size }) },
    evolutionRecords.length > 0 && { id: 'evolution', label: t('Evolução') },
    versionRecords.length > 0 && { id: 'versions', label: t('Versões') },
    entry.description && { id: 'description', label: t('Descrição') },
  ].filter(Boolean)
    ...
    <div className="detail-page" style={{ '--detail-accent': detailAccent }}>
      ...
      <section className={`detail-hero ${isPokelogOnly ? 'pokelog-only' : ''}`}>
        ...
        <div className="detail-intro">
          <div className="eyebrow"><Sparkles size={14} />{clans.join(' · ') || t('Sem classificação de clan')}</div>
          <h1>{name}</h1>
          {entry.page_title !== name && <p className="source-title">{t('Página: {title}', { title: entry.page_title })}</p>}
          <div className="badge-row large">{elements.map((element) => <ElementBadge key={element} element={element} />)}</div>
          <div className="hero-meta-row">
            {levels.length > 0 && <span><Clock3 size={16} />{t('Level {levels}', { levels: levels.join(' / ') })}</span>}
            {tiers.length > 0 && <span><Crown size={16} />{t('Tier {tiers}', { tiers: tiers.join(' · ') })}</span>}
            {clans.length > 0 && <span><Swords size={16} />{clans.length === 1 ? t('{count} clan', { count: clans.length }) : t('{count} clans', { count: clans.length })}</span>}
            {capture?.difficulty?.label && <span><Gauge size={16} />{t('Captura {label}', { label: capture.difficulty.label })}</span>}
            {hasTasks && <span><ClipboardList size={16} />{new Set(taskOccurrences.map((occurrence) => occurrence.task_id)).size === 1 ? t('{count} task', { count: new Set(taskOccurrences.map((occurrence) => occurrence.task_id)).size }) : t('{count} tasks', { count: new Set(taskOccurrences.map((occurrence) => occurrence.task_id)).size })}</span>}
          </div>
          <div className="detail-source-row">
            <span className={`data-availability ${isPokelogOnly ? 'partial' : 'complete'}`}><i />{isPokelogOnly ? t('Somente no Pokélog') : t('Ficha publicada')}</span>
            {!isPokelogOnly && <SourceLink href={entry.source_url}>{t('Abrir ficha original')}</SourceLink>}
            {entry.animated_model?.source_page_url && <SourceLink href={entry.animated_model.source_page_url}>{t('Modelo animado')}</SourceLink>}
          </div>
        </div>
      </section>

      {isPokelogOnly && (
        <div className="availability-notice"><ClipboardList size={19} /><div><strong>{t('Ficha individual ainda não publicada')}</strong><p>{t('Esta forma foi catalogada no compilado oficial do Pokélog. Por isso, dados de clan, movimentos e efetividades podem não estar disponíveis.')}</p></div></div>
      )}

      <nav className="detail-section-nav" aria-label={t('Seções da ficha')}>
        <span>{t('Nesta ficha')}</span>
      ...
      <div className={`detail-grid ${hasSidebar ? '' : 'no-sidebar'}`}>
        <div className="detail-main">
          ...
          <Section id="combat" title={t('Clans e funções')} icon={<Swords size={18} />} description={t('Cada card preserva a classificação e a disponibilidade publicadas para PvE e PvP.')}>
            {entry.clan_memberships?.length > 0 ? (
              ...
              <EmptyDetailState icon={<Swords size={20} />} title={t('Sem classificação na tabela de clans')}>{t('Isso não significa que o Pokémon seja inutilizável; apenas não há tier ou função competitiva publicada para ele.')}</EmptyDetailState>
            )}
          </Section>
          ...
          {evolutionRecords.length > 0 && (
            <Section id="evolution" title={t('Linha evolutiva')} icon={<Layers3 size={18} />} description={t('Use os cards para navegar quando a forma também estiver na base.')}>
              <div className="evolution-line">
                {evolutionRecords.map((evolution, index) => {
                  ...
                  const content = <><PokemonImage src={evolution.image_url || (evolution.match && pokemonImage(evolution.match))} name={evolution.name} /><div className="evolution-info"><span>{t('Estágio {index}', { index: String(index + 1).padStart(2, '0') })}</span><strong>{evolution.name}</strong><small>{evolutionLevel ? t('Level {level}', { level: evolutionLevel }) : t('Level não informado')}</small>{evolutionElements.length > 0 && <div>{evolutionElements.slice(0, 2).map((element) => <ElementBadge key={element} element={element} compact />)}</div>}</div></>
                  return evolution.match
          ...
          {versionRecords.length > 0 && (
            <Section id="versions" title={t('Outras versões')} icon={<Sparkles size={18} />} description={t('Compare rapidamente level, elementos e classificação de cada forma.')}>
              <div className="versions-grid">
                {versionRecords.map((version) => {
                  ...
                  const content = <><PokemonImage src={version.image_url || (version.match && pokemonImage(version.match))} name={version.name} /><div className="version-info"><strong>{version.name}</strong><span>{versionLevels.length ? `Lv ${versionLevels.join(' / ')}` : t('Level —')}{versionTiers.length ? ` · ${versionTiers.map(tierLabel).join(' · ')}` : ''}</span><div>{versionElements.slice(0, 2).map((element) => <ElementBadge key={element} element={element} compact />)}</div></div></>
                  return version.match
          ...
          {entry.description && <Section id="description" title={t('Descrição da Wiki')} icon={<BookOpen size={18} />}><p className="description-text">{entry.description}</p></Section>}
          ...
          <aside className="detail-sidebar">
            {hasGeneralInfo && (
              <section className="side-card">
                <h2>{t('Ficha rápida')}</h2>
                <InfoItem label={t('Nome')}>{info.name}</InfoItem>
                <InfoItem label={t('Level')}>{info.level}</InfoItem>
                <InfoItem label={t('Boost')}>{info.boost}</InfoItem>
                <InfoItem label={t('Matéria')}>{info.matter}</InfoItem>
                <InfoItem label={t('Pedra de evolução')}>{info.evolution_stone}</InfoItem>
                <InfoItem label={t('Mega Stone')}>{info.mega_stone}</InfoItem>
                <InfoItem label={t('Item de evolução')}>{info.evolution_item}</InfoItem>
              </section>
            )}

            {entry.level_entries?.length > 0 && (
              <section className="side-card">
                <h2>{t('Ocorrências por level')}</h2>
                <div className="level-occurrences">
                  {entry.level_entries.map((level, index) => <div key={`${level.section}-${index}`}><strong>{t('Level {level}', { level: level.level })}</strong><span>{level.display_name}</span><small>{level.accessible ? t('Acessível') : level.note || t('Não acessível')}</small></div>)}
                </div>
              </section>
            )}
            ...
            {entry.variants?.length > 0 && (
              <section className="side-card">
                <h2>{t('Variantes')}</h2>
                {entry.variants.map((variant, index) => <InfoItem key={`${variant.name}-${index}`} label={variant.name || t('Variante {index}', { index: index + 1 })}>{t('Level {level}', { level: variant.level ?? '—' })}</InfoItem>)}
              </section>
            )}
          </aside>
        )}
