...
function ItemLink({ item }) {
  return <Link className="relation-item" to={`/items/${encodeURIComponent(item.id)}`}>{item.image_url && <img src={item.image_url} alt="" />}<span><strong>{item.name}</strong><small>{item.categories?.[0] || 'Item'}</small></span></Link>
}
...
export default function ItemDetailPage() {
  ...
  const loading = catalog.loading || crafting.loading
  const error = catalog.error || crafting.error
      ...
      {!item ? <div className="catalog-empty page-frame"><Package size={34} /><strong>Item não encontrado</strong><Link to="/items">Voltar ao ItemDex</Link></div> : (
        <div className="item-detail-page page-frame">
          ...
          <div className="item-detail-grid">
            ...
            <section className="relation-panel"><header><h2>Dropado por</h2><b>{item.dropped_by?.length || 0}</b></header><div className="drop-links">{item.dropped_by?.map((name) => <Link to={`/pokemon/${encodeURIComponent(name.replace(/\s+/g, '_'))}`} key={name}>{name}</Link>)}</div>{!item.dropped_by?.length && <p className="relation-empty">Nenhuma relação direta de drop publicada nesta tabela da Wiki.</p>}</section>
